  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        'https://rldondon.pythonanywhere.com/piggyboxw',
        formData,
        {
          headers: {
    ...
    <>
      <div className='bg-[#2D2A32]/50 rounded-lg p-5 h-[100%] w-full absolute flex flex-col gap-5'>
        <div className='function flex items-center'>
          ...
          <span className=' w-fit p-3 rounded-lg'>Withdraw from a piggybox</span>
        </div>

        <form onSubmit={handleSubmit}>
          <div className='accounts_form gap-5 flex flex-col h-fit min-w-[450px] w-[100%] rounded-lg'>
          <div className='field piggybox'>
              <label htmlFor='box'>Piggybox</label>
              <input type='text' id='box' name='box' value={formData.box} onChange={handleChange} />
            </div>
